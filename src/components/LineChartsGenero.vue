...
<template>
  <div class="row candidato-wrapper">
    ...
    <div class="col-12 d-block d-md-none text-center">
      <b-dropdown class="m-3 d-inline-block justify-content-center" id="dropdown-form" text="Seleccionar región policiaca" ref="dropdown" variant="dark">
        <b-dropdown-form>
          <b-form-group>
            <template #label>
              <b-form-checkbox
                ...
                @change="toggleAll"
              >
                  {{ selected.length > 0 ? ' Todas las regiones' : ' Seleccionar todos'}}
              </b-form-checkbox>
    ...
    <div class="col-12 col-md-8 text-center" id="#my_dataviz">
      <svg height="350px" width="100%" ref="dataviz">
        <g ref="g"></g>
      </svg>
      <div id="tooltip"></div>
    </div>
    ...
    <div class="col-4 d-none d-md-block">
      <div class="row mt-3">
        <div class="col-12">
          <p class="p-3">Evolución de incidentes</p>
        </div>        
      </div>
      <b-form-group class="lista-regiones">
        <template #label>
          <b-form-checkbox
            v-model="checked"				
            class="check mt-3 mb-3"
            aria-describedby="region_policiaca"
            aria-controls="region_policiaca"
            @change="toggleAll"
          >
    ...
    methods: {
      ...
      renderGraph(dataset) {
        ...
        var margin = {top: 50, right: 20, bottom: 30, left: 50},
          width = (canvas_size - (canvas_size * .40)) - margin.left - margin.right,
          height = 350 - margin.top - margin.bottom;

        if(window.innerWidth < 640)
          width = width = (canvas_size- 90) - margin.left - margin.right
        ...
        let maxIncidentes = maxBy(dataset, 'maxItem')
          ...
          if(dataset.length > 0) {
            ...
            let last_position = find(candidato.items, ['anio', `2021`]).incidentes
            ...
            map(candidato.items, i => {
              svg.append("text")
                .attr("transform", () => {
                  ...
                  return "translate(" + x(i.anio) + "," +( y(i.incidentes) - 5) + ")"
                })
                .attr("dy", ".35em")
                .attr("text-anchor", "start")
                .attr('class', 'label-line')
                .style("fill", this.colors[i.area_id])
                .text(() => {
                  return numeral(i.incidentes).format('0,0')
                })
            })
