...
<template>
  <div class="row candidato-wrapper">    
    <div class="col-12 col-md-8 text-center" id="#my_dataviz_ataques">
      <svg height="350px" width="100%" ref="dataviz_ataques">
        <g ref="g_ataques"></g>
      </svg>
      <div id="tooltip"></div>
    </div>
    ...
    <div class="col-12 d-block d-md-none">
        ...
        <b-dropdown class="m-3 justify-content-center" id="dropdown-form" text="Seleccionar región policiaca" ref="dropdown">
          <b-dropdown-form>
            <b-form-group>
              <template #label>
                <b-form-checkbox
    ...
    <div class="col-4 d-none d-md-block">
      <div class="row">
        <div class="col-12">
      <p class="p-3">Querellas por violencia doméstica según el tipo de maltrato contra la mujer</p>
    </div>
      ...
      <b-form-group class="lista-regiones">
        <template #label>
          <b-form-checkbox
            ...
            @change="toggleAll"
          >
             {{ selected.length > 0 ? ' Tipo de maltrato' : ' Seleccionar todos'}}
          </b-form-checkbox>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        all_incidentes: state => state.incidentes.tipoAtaques,
      }), 
      ...
      colors() {
        return {
          ...
          'fisico': '#1f78b4',
          'otro':'#b2df8a',
          'psicologico-o-emocional':'#33a02c',
    ...
    methods: {
      ...
      renderGraph(dataset) {
        ...
        var margin = {top: 50, right: 20, bottom: 30, left: 50},
          width = (canvas_size - (canvas_size * .25)) - margin.left - margin.right,
          height = 350 - margin.top - margin.bottom;

        if(window.innerWidth < 640)
          width = width = (canvas_size- 60) - margin.left - margin.right

        var svg = d3.select(this.$refs['g_ataques'])
          .attr("transform",
